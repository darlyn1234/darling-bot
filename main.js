...
async function starts() {
	...
	Fg.on('qr', () => {
	console.log(color('[QR]','white'), color('Escanea el codigo QR para conectarte'));
	});
  ...
  Fg.on('group-participants-update', async (anu) => {
      ...
      if (anu.action == 'add') {
	    ...
	    capt = welc.replace('@user', tag).replace('@name', username).replace('@bio', about).replace('@date', tanggal).replace('@desc', descrip).replace('@group', mdata.subject);
	      Fg.send2ButtonLoc(mdata.id, buff, capt, '𝓢𝓸𝓯í𝓪\n८≛ 𝓓𝓪𝓻𝓵𝔂𝓷 𔐬☆᤻᤻[𝓼𝓮𝔁]', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: {  
        ...
        Fg.sendButtonLoc(mdata.id, buff, capt, '𝓢𝓸𝓯í𝓪\n८≛ 𝓓𝓪𝓻𝓵𝔂𝓷 𔐬☆᤻᤻[𝓼𝓮𝔁]', 'Adios xd', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Fg.parseMention(capt)
	      } 
	    });
...
Fg.on('group-participants-update', async (anu) => {
  ...
  try {
	      ...
	      ppimg = 'https://telegra.ph/file/f10eec03d242282d8d13e.jpg';
	    } 
...
Fg.on("CB:Call", json => {
  ...
  call = calling[1].from;
  Fg.sendMessage(call, `*${Fg.user.name}* No hagas llamadas al bot chistoso/a, tu número se bloqueará automáticamente\n\n𝓢𝓸𝓯í𝓪\n८≛ 𝓓𝓪𝓻𝓵𝔂𝓷 𔐬☆᤻᤻[𝓼𝓮𝔁]`, MessageType.text).then(() => Fg.blockUser(call, "add"));
}); 
...
require('./index.js');
nocache('./index.js', module => console.log(color(`El Index.js gracias a Darlyn`)));
